...
class Quack extends Renderer {
    ...
    protected function parseUse(&$content){
        ...
        if (preg_match(self::REGEX_USE, $content, $matches)){
            ...
            $paths = array();
            $paths['index'] = df_APP_ROOT . df_DS . 'views' . df_DS . $matches[1] . '.tpl.html';
            $paths['mod'] = ($this->TPL->getModule()) ? df_APP_ROOT . df_DS . 'modules' . df_DS . $this->TPL->getModule() . df_DS . 'views' . df_DS . $matches[1] . '.tpl.html' : false;
